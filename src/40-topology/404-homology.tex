...
\begin{itemize}
\item (normalizacja) $K(\LittleUnknot) = q + 1/q$;
\item (stabilizacja) $K(L \sqcup \LittleUnknot) = (q + 1/q) K(L)$;
\item (relacja kłębiasta) \begin{equation}
    q^{-2} K\left(\MediumSkeinPlus\right) - q^{2} K\left(\MediumSkeinMinus\right) = (q^{-1}-q) K\left(\MediumSkeinZero\right).
\end{equation}
\end{itemize}
...
\begin{proof}
    ...
    W pierwszym panowie pokazują, że istnieje ciąg spektralny zaczynający się od zredukowanej kohomologii Chowanowa, po którym następuje koniec: homologia zdefiniowana singularnymi instantonami.
